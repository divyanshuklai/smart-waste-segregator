...
A camera on the lid takes one photo when a VL53L0X LIDAR sensor is tripped. A
MobileNetV3-Small runs on a Raspberry Pi 4 under onnxruntime and picks a class. Two
servo-driven flaps route the item. Built November 2024; I came back to it in March 2025
for the YOLO11 exports.
...
Two flaps in series give four outlets from two binary decisions, so a class is just a
pair of servo positions: plastic is right then left, glass left then left, metal right
then right, organic left then right.
